...
class TomorrowAdapter(private val list: List<Forecast>): RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    inner class HourlyTitleItem(private val titleBinding: HourlyForecastTitleItemBinding): RecyclerView.ViewHolder(titleBinding.root) {
        ...
        fun bindTitleItem(item: Forecast.TitleForecast){
            titleBinding.apply {
                ...
                dateTextView.text = (
                                item.domainHourlyForecast.date.dayOfWeek.getDisplayName(TextStyle.FULL, Locale.getDefault()) + " " +
                                item.domainHourlyForecast.date.dayOfMonth.toString() + " " +
                                item.domainHourlyForecast.date.month.getDisplayName(TextStyle.FULL, Locale.getDefault())
                            ).lowercase()
            }
        }
    }

    inner class HourlyListItem(private val listBinding: HourlyForecastListItemBinding): RecyclerView.ViewHolder(listBinding.root) {
        @SuppressLint("SetTextI18n")
            ...
            listBinding.apply {
                ...
                expandableButtonImageView.setOnClickListener {
                    if(hiddenHourlyForecastListItemView.visibility == View.VISIBLE){
                        TransitionManager.beginDelayedTransition(hourlyForecastListItemView, AutoTransition())
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when(viewType){
            ...
            else -> {TODO(error("onCreateViewHolder"))}
        }
    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when(getItemViewType(position)){
            ...
            else -> {TODO(error("onBindViewHolder"))}
        }
    }
